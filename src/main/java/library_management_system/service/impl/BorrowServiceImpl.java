...
import library_management_system.exception.BusinessRuleValidationException;
...
public class BorrowServiceImpl implements BorrowService {
	...
	public BorrowResponse borrowBook(BorrowRequest request) {
		...
		if (!book.isActive()) {
			throw new BusinessRuleValidationException("Book is not available (inactive)");
		}

		if (borrowRepository.existsByBookIdAndStatus(book.getId(), BorrowStatus.ACTIVE)) {
			throw new BusinessRuleValidationException("Book is already borrowed by another member");
		}
	...
	public BorrowResponse returnBook(UUID borrowId) {
		...
		if (borrow.getStatus() != BorrowStatus.ACTIVE) {
			throw new BusinessRuleValidationException("This book has already been returned");
		}
