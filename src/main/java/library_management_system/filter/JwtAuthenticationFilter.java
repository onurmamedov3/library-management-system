package library_management_system.filter;
...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
	...
	protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
		...
		if(!checkIfTokenIsAccessToken(token)){
			reject(response, "Expired token");
			return;
		}
	...
	private void reject(HttpServletResponse response, String message) throws IOException{
		...
		ErrorResponse errorResponse = new ErrorResponse(HttpServletResponse.SC_UNAUTHORIZED, "Unauthorized", message);
